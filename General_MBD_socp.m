function [OptX,OptY,OptValue,k,delta_T,delta_T_MP] = General_MBD_socp(C,D,A,B,b,E,h,F,r_le,G,r_ls,Qs,gs,ls,n_block,n_y)
...
% iii) Linear independence: with given discrete variables x, groups of continuous variables y are linearly independent.
...
% 
% % Specify the non-conic part of the problem.
% prob.c = [D' zeros(1,(n_y*n_block+1)*n_block) ];
% a_socp = zeros(n_y*n_block*n_block,(n_y*n_block+1)*n_block + n_y*n_block);
% for j = 1 : n_block
%     a_socp(n_y*n_block*(j-1)+1:n_y*n_block*j,1:n_y*n_block) = -sqrt(Qs{j})   ;
%     a_socp(n_y*n_block*(j-1)+1:n_y*n_block*j,n_y*n_block+(n_y*n_block+1)*(j-1)+1:n_y*n_block+(n_y*n_block+1)*j) =  [ zeros(n_y*n_block,1) eye(n_y*n_block,n_y*n_block) ];
% end
% prob.a = sparse([B zeros(nRowB,(n_y*n_block+1)*n_block) ;
%     E zeros(nRowE, (n_y*n_block+1)*n_block  );
%     a_socp]);
% blc_socp =[];
% for j = 1 : n_block
%     blc_socp = [   blc_socp ;0.5.*( inv(sqrt(Qs{j})) )'*ls(:,j)];
% end
% prob.blc = [-inf.*ones(size(b,1),1);h; blc_socp];
% prob.buc = [b - A*x0 ;h; blc_socp];
% blx_socp = [];
% for j = 1 : n_block
%     blx_socp = [ blx_socp  sqrt(0.25.*ls(:,j)'*Qs{j}'*ls(:,j) +gs(j,1))  -inf.*ones(1,n_y*n_block) ];
% end
% prob.blx = [zeros(n_y*n_block,1)',   blx_socp ];
% bux_socp = [];
% for j = 1 : n_block
%     bux_socp = [ bux_socp  sqrt(0.25.*ls(:,j)'*Qs{j}'*ls(:,j) + gs(j,1))  inf.*ones(1,n_y*n_block) ];
% end
% prob.bux = [inf*ones(n_y*n_block,1)',bux_socp];
% % Specify the number of cones.
% prob.cones = cell( n_block ,1);
% for j = 1 : n_block
%     prob.cones{j}.type = 'MSK_CT_QUAD';      % MSK_CT_QUAD for Rotated Qudratic Cone
%     prob.cones{j}.sub = [n_y*n_block + (n_y*n_block+1)*(j-1)+1, n_y*n_block + 2 + (n_y*n_block+1)*(j-1):n_y*n_block + n_y*n_block + 1 + (n_y*n_block+1)*(j-1)];
% end
% [r1,res]=mosekopt('minimize echo(0)',prob,param);
% OptY = res.sol.itr.xx(1:n_y*n_block,1);
