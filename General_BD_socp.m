...
function [OptX,OptY,OptValue,k,delta_T,delta_T_MP] = General_BD_socp(C,D,A,B,b,E,h,F,r_le,G,r_ls,Q,g,l)
...
while k<kmax
    ...
    a_socp = sparse(zeros(n_y*n_socp,(n_y+1)*n_socp + n_y));
    for j = 1 : n_socp
        a_socp(n_y*(j-1)+1:n_y*j,1:n_y) = -sqrt(Q{j})   ;
        a_socp(n_y*(j-1)+1:n_y*j,n_y+(n_y+1)*(j-1)+1:n_y+(n_y+1)*j) =  sparse([ zeros(n_y,1) eye(n_y,n_y) ]);
    end
    ...
    blx_socp = [];
    for j = 1 : n_socp
        blx_socp = [ blx_socp  sqrt(0.25.*l(:,j)'*Q{j}'*l(:,j) + g(j,1))  -inf.*ones(1,n_y) ];
    end
    prob.blx = [zeros(n_y,1)',   blx_socp ];
    bux_socp = [];
    for j = 1 : n_socp
        bux_socp = [ bux_socp  sqrt(0.25.*l(:,j)'*Q{j}'*l(:,j) + g(j,1))  inf.*ones(1,n_y) ];
    end
    ...
    if strcmp(res.sol.itr.solsta, 'OPTIMAL')
       ...
       ff = 0;
        for j = 1 : n_socp
            ff =  ff + res.sol.itr.snx( (j-1)*(n_y+1)+n_y+1,1)*( g(j,1) - (yf0(p,:)*Q{j}*yf0(p,:)' + l(:,j)'*yf0(p,:)'));
        end
    ...
    elseif strcmp(res.sol.itr.solsta, 'PRIMAL_INFEASIBLE_CER')
        ...
        dd = 0;
        for j = 1 : n_socp
            dd =  dd + res.sol.itr.snx( j*(n_y+1)+1,1)*( g(j,1) - (y0(q,:)*Q{j}*y0(q,:)' + l(:,j)'*y0(q,:)')) ...
...
%     prob.cones{j}.sub = [n_y + (n_y+1)*(j-1)+1, n_y + 2 + (n_y+1)*(j-1):n_y + n_y + 1 + (n_y+1)*(j-1)];
% end
% [r1,res1]=mosekopt('minimize echo(0)',prob,param);
% OptY = res1.sol.itr.xx(1:n_y,1);
